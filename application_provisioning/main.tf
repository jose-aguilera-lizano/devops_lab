...
resource "aws_instance" "webserver_01" {
  ...
  provisioner "local-exec" {
    command = "ANSIBLE_HOST_KEY_CHECKING=\"False\" ansible-playbook -i '${self.public_ip},' test.yml --extra-vars 'rds_endpoint=\"${aws_db_instance.lab_rds.address}\" rds_user=\"${aws_db_instance.lab_rds.username}\" rds_password=\"${aws_db_instance.lab_rds.password}\" rds_database=\"${aws_db_instance.lab_rds.name}\"'"
  }
